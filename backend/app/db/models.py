...
Full relational model documented in docs/11-database.md. Model names in this
module are the single source of truth for migrations and services.
"""
...
class WorkspaceMember(Base):
    ...
    role: Mapped[Role] = mapped_column(Enum(Role, name="role_enum"), default=Role.MEMBER)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())
...
class Document(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    workspace_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("workspaces.id", ondelete="CASCADE"), index=True)
    uploaded_by: Mapped[uuid.UUID] = mapped_column(ForeignKey("users.id", ondelete="SET NULL"))
